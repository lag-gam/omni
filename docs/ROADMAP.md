...
`lib/intent.ts`: one Claude call classifies input as `SAVE` / `RECALL` /
`CHAT`. This is the phase to spend real time on — write down 15–20 example
inputs (statements, questions, statement-shaped-like-questions) and check
the classifier against all of them before moving on.
`feat: intent classification layer`

## Phase 6 — Recall
On `RECALL`, run the similarity search from Phase 4 and ask Claude to answer
using only the retrieved notes — including saying plainly when nothing
relevant is stored. Wire this into `/api/capture` alongside the save path.
`feat: semantic recall and answer synthesis`

## Phase 7 — Unified ambient flow
Collapse save and recall into the one response area under the omni-bar: a
saved note gets a quiet acknowledgment, a question gets its answer, in the
same spot, same styling. No branching UI.
`feat: unified response flow`

## Phase 8 — Voice capture workflow
